...
class Driver extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $statusx = html_escape($this->input->post('status', TRUE));
        $area = html_escape($this->input->post('area', TRUE));
        $ap = html_escape($this->input->post('ap', TRUE));
        $stringx = html_escape($this->input->post('string', TRUE));
        $status='1';
        $string='';
        ...
        if(!empty($stringx)){
            ...
            $status='3';
        } elseif(empty($statusx)){
            $status='10';
            $statusjob='10';
        } else {
            $statusjob = $statusx;
        }
        $data['driver'] = $this->driver->getAlldriverby($job, $status, $statusjob , $area, $string,$ap);
        $data['hasil'] = $area.",".$job.",".$statusjob.",".$string;
    ...
    public function tracking_driver_bike()
    {
        $this->load->view('includes/header');
        $this->load->view('drivers/tracking_driver_bike');
    }
    ...
    public function detail($id)
    {
        ...
        $data['wallettrx'] = $this->driver->wallettrx($id);
    ...
    public function ubahid()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            $ca = html_escape($this->input->post('alamat_driver', TRUE));
            $cax = $this->dashboard->getcountryareaby($ca) ;
            $caname = $cax['area_name'];
            ...
            $data             = [
                ...
                'pool'                  => html_escape($this->input->post('pool', TRUE)),
                'berkah'                => html_escape($this->input->post('berkah', TRUE)),
                'kanigara'              => html_escape($this->input->post('kanigara', TRUE)),
                'smartrans'             => html_escape($this->input->post('smartrans', TRUE)),
                'ayuindah'              => html_escape($this->input->post('ayuindah', TRUE)),
                'rent'                  => html_escape($this->input->post('rent', TRUE)),
                'kurir'                 => html_escape($this->input->post('kurir', TRUE)),
                'ap'                 => html_escape($this->input->post('ap', TRUE)),
                  'paskas'                 => html_escape($this->input->post('paskas', TRUE)),
                'alamat_driver'         => $caname,
                'region_code'           => $ca
            ];
    ...
    public function tambah()
    {
        ...
        $area = html_escape($this->input->post('alamat_driver', TRUE));
        switch ($area) {
            case "JABODETABEK":
                $region_code = 11;
            break;
        ...
        if ($this->form_validation->run() == TRUE) {
            ...
            if ($this->form_validation->run() == TRUE) {
                if (@$_FILES['foto_stnk']['name']) {
                    ...
                    if ($this->upload->do_upload('foto_stnk')) {
                        ...
                        $fotostnk = html_escape($this->upload->data('file_name'));
                    } else {
                        $fotostnk = 'noimage.jpg';
                    }
                }
            }
            ...
            $kata = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890';
            $refferal = '';
            for($i = 0; $i < 5; $i++) {
            ...
            $datasignup             = [
                ...
                'reg_id'                    => 'R' . time(),
                'foto'                      => $foto,
                'password'                  => '0b5615eb88c6852c11828ec64a9d9d4ba95c26ec',
                'nama_driver'               => html_escape($this->input->post('nama_driver', TRUE)),
                'no_telepon'                => str_replace("+", "", $countrycode) . $phone,
                'email'                     => html_escape($this->input->post('email', TRUE)),
                'gender'                    => html_escape($this->input->post('gender', TRUE)),
            ...
            $databerkas             = [
                ...
                'foto_ktp'                  => $fotoktp,
                'foto_stnk'                 => $fotostnk,
                'stnk_berlaku'              => html_escape($this->input->post('masa_berlaku', TRUE)),
                'id_sim'                    => html_escape($this->input->post('id_sim', TRUE))
