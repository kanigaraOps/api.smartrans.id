...
return false;
    ...
    {
        $cek = $this->db->query("SELECT token_ap FROM app_settings Orders LIMIT 1");
        if ($cek->num_rows() == 1) {
            return $cek->row();
        } else {
            return false;
    ...
    {
        $post_ap=$this->post_ap($data);
        if($post_ap->status ==true){
            return $post_ap;
        }elseif($post_ap->status==false and $post_ap->message == "Token Time Expire."){
           $this->login_ap();
           $post_ap=$this->post_ap($data);
        if($post_ap->status ==true){
            return $post_ap;
    ...
    public function post_ap($data)
    {$post =(object) ["store"=> [
(object)[
"store_id"=> $data->id_terminal_ap,
"transactions"=>[
    (object)[
        "invoice_no"=> $data->id_order,
        "trans_date"=> date('Y-m-d'),
        "trans_time"=> date('Y-m-d H:i:s'),
        "sequence_unique"=> $data->id_order,
        "item_name"=> $data->alamat_asal,
        "item_code"=> $data->id_order,
        "item_qty"=> "1",
"item_price_per_unit"=> (int)$data->total/100,
"item_price_amount"=> (int)$data->total/100,
"item_vat"=> "0",
"item_total_price_amount"=> (int)$data->total/100,
"item_total_vat"=> "0",
"transaction_amount"=> (int)$data->total/100,
"currency"=> "IDR"]]]]];
     $ch = curl_init();

curl_setopt($ch, CURLOPT_URL,"https://api-ecsys.angkasapura2.co.id/api/v1/transaction/");
curl_setopt($ch, CURLOPT_POST, 1);
curl_setopt($ch, CURLOPT_POSTFIELDS,json_encode($post));
$header = array();
$header[] = 'Content-type: application/json';
$header[] = 'Authorization:'.$this->get_token()->token_ap;
...
$server_output = curl_exec($ch);
 return json_decode($server_output);
    ...
    public function nearby( $latitude2, $longitude2, $unit = 'kilometers') {
            $latitude1=-6.123493;
    ...
    public function get_driver_ap()
    {
        $stat =  $this->db->query("SELECT id FROM driver WHERE ap='1'");
           return $stat->result_array();
    }
    public function updete_laporan_bukan_member($id)
    {

        $this->db->where('id_transaksi', $id);
        return $this->db->update('laporan_ap', array('filter' => 1,'keterangan' => 'Driver Cadangan','finish' => 1));
    }
    public function updete_laporan_finish_ap($id)
    {

        $this->db->where('id_transaksi', $id);
        return $this->db->update('laporan_ap', array('finish' => 1));
    }
    public function updete_laporan_report_ap($id)
    {

        $this->db->where('id_transaksi', $id);
       return $this->db->update('laporan_ap', array('report' => 1,'finish' => 1));
        
    }
    public function get_laporan_apid($id)
    {
        return $this->db->query("SELECT laporan_ap.* ,transaksi.id_order ,transaksi.alamat_asal FROM laporan_ap LEFT JOIN transaksi ON transaksi.id = laporan_ap.id_transaksi WHERE laporan_ap.id_transaksi='$id'")->row();
    }
    public function get_laporan_report_ap($tanggal=null)
    {
        $tanggal==null?$tanggal=date('Y-m-d'):$tanggal;
        $stat =  $this->db->query("SELECT  *  FROM laporan_ap WHERE finish='1' AND report='1' AND filter!='1' AND tanggal='$tanggal'")->result_array();
       
            return $stat;
    }
    public function get_laporan_ap($tanggal=null)
    {
        $tanggal==null?$tanggal=date('Y-m-d'):$tanggal;
        $stat =  $this->db->query("SELECT  *  FROM laporan_ap WHERE finish='1' AND filter!='1' AND tanggal='$tanggal'")->result_array();
            return $stat;
    }
    public function cek_laporan_ap($id,$tgljemput=null)
    {   
        $tgljemput==null?$tgljemput=date('Y-m-d'):$tgljemput;
        $stat =  $this->db->query("SELECT  *  FROM laporan_ap WHERE laporan_ap.id_transaksi='$id' AND filter ='0' and laporan_ap.tanggal = '$tgljemput'");
        if ($stat->num_rows() == 1) {
            return true;
        } else {
            return false;
    ...
    public function cek_driver_ap($id)
    {
        $stat =  $this->db->query("SELECT * FROM driver WHERE ap='1' AND id='$id'");
        if ($stat->num_rows() == 1) {
            return true;
        } else {
            return false;
    ...
    public function get_job_services()
    {
     ...
     public function get_driver_regidall()
    {
    ...
    function my_location($data_l)
    {
        ...
        $status = $this->db->get('config_driver')->row('status'); 
    ...
    //testing
    public function get_deduct_timerx()
    {
        $todayx = date('Y-m-d h:i:s',strtotime("-1 days"));
        $this->db->where('id' , 'D0000000222');
        $this->db->update('driver', array('nama_driver' => $todayx)); 
        if ($this->db->affected_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    {
        $this->db->select('driver.id, saldo.saldo');
        $this->db->from('driver');
        $this->db->join('config_driver', 'driver.id = config_driver.id_driver');
        $this->db->join('saldo', 'driver.id = saldo.id_user'); 
        $this->db->where('driver.region_code','11');
        $this->db->where('driver.pool','1');
    ...
    function addantrian($id)
    {
        ...
        $driver = $this->db->get()->result_array();
        if(0!=count($driver)){
            $this->db->where('id', $driver[count($driver)-1]['id']);
            $data = array(
                'id_driver' => $id,
                'status_antrian' => 0,
                'terminal' => 0
    ...
    function cancelantrian($id)
    {
        ...
        $this->db->where('id_driver', $id);
        $this->db->where('status_antrian', '0');
        $this->db->where('lokasi != 1');
        $edit = $this->db->update('antrian', $data);
        if ($this->db->affected_rows() > 0) {
            return true;
        } else {
            return false;
    ...
    {
        $this->db->select('*, transaksi.id as id');
        $this->db->from('transaksi');
        ...
        $this->db->order_by('history_transaksi.status ASC, transaksi.waktu_order DESC');
        $trans = $this->db->get();
        return $trans;
    }
    
    function all_transaksi_periode($iduser, $periode)
    {
        ...
        $this->db->where('history_transaksi.status != 0');
        if($periode == 'harian'){
            $this->db->where('transaksi.waktu_order =', $today);
        } else if($periode == 'mingguan'){
            $this->db->where('transaksi.waktu_order <=', $today);
            $this->db->where('transaksi.waktu_order >=', $today7);
        }
        $this->db->order_by('history_transaksi.status ASC, transaksi.waktu_order DESC');
        $trans = $this->db->get();
        return $trans;
    ...
    function book_history($iddriver)
    {
        ...
        if($berkah == 1 && $region_code==11 && $saldo>=0){
            return $trans;
       ...
       function book_historytest($iddriver)
    {
        ...
        $ayuindah = $driver->row('ayuindah');
        $saldo = $driver->row('saldo');
         $newtrans =$this->db->query("
        select a.*,b.*,c.*,d.status_transaksi,e.* 
        from transaksi a 
        left join transaksi_detail_merchant b on a.id=b.id_transaksi 
        ...
        where a.id_driver='D0'   and (c.status ='0'or c.status ='1') order by d.status_transaksi DESC")->result_array();;
        ...
        left join status_transaksi d on c.status=d.id 
        left join fitur e on a.order_fitur=e.id_fitur 
        where a.id_driver='$iddriver' and a.now_later='2' and a.region_code='$region_code' and c.status ='12' order by d.status_transaksi DESC")->result_array();
        // select a.*,b.*,c.*,d.status_transaksi,e.* 
        // from transaksi a 
        // left join transaksi_detail_merchant b on a.id=b.id_transaksi 
        // left join history_transaksi c on a.id=c.id_transaksi 
        // left join status_transaksi d on c.status=d.id 
        // left join fitur e on a.order_fitur=e.id_fitur 
        // where a.id_driver='$iddriver' and a.now_later='2' and a.region_code='$region_code' and c.status ='12' order by d.status_transaksi DESC
        $newtrans = array_merge($newtrans,$assigned);
        ...
        left join status_transaksi d on c.status=d.id 
        left join fitur e on a.order_fitur=e.id_fitur 
        where a.id_driver is null and a.now_later='2' and a.region_code='$region_code' and a.fleet = 'Kanigara Trans' and c.status ='0' order by d.status_transaksi DESC")->result_array();
        $newtrans = array_merge($newtrans,$new);
        } 
        ...
        left join status_transaksi d on c.status=d.id 
        left join fitur e on a.order_fitur=e.id_fitur 
        where a.id_driver is null and a.now_later='2' and a.region_code='$region_code' and a.fleet = 'Smartrans' and c.status ='0' order by d.status_transaksi DESC")->result_array();
        $newtrans = array_merge($newtrans,$new2);
        } 
        ...
        left join status_transaksi d on c.status=d.id 
        left join fitur e on a.order_fitur=e.id_fitur 
        where a.id_driver is null and a.now_later='2' and a.region_code='$region_code' and a.fleet = 'Ayu Indah' and c.status ='0' order by d.status_transaksi DESC")->result_array();
        $newtrans = array_merge($newtrans,$new3);
        } 
         return $newtrans;
        if($berkah == 1 && $region_code==11 && $saldo>=0){
            return $newtrans;
            ...
            $newtrans = $this->db->query("select a.*,b.*,c.*,d.status_transaksi,e.* from transaksi a left join transaksi_detail_merchant b on a.id=b.id_transaksi left join history_transaksi c on a.id=c.id_transaksi left join status_transaksi d on c.status=d.id left join fitur e on a.order_fitur=e.id_fitur where a.id_driver is null and a.now_later='2' and a.region_code='100'and c.status ='0' order by d.status_transaksi DESC");
            return $newtrans;
        }
    }
    
    function book_historyxx($iddriver)
    {
        ...
        // echo "berkah : ". $berkah . " kanigara : ". $kanigara . " smartrans : ". $smartrans . " ayuindah : ". $ayuindah ." ----- ";
        ...
        // left join status_transaksi d on c.status=d.id 
        // left join fitur e on a.order_fitur=e.id_fitur 
        // where a.id_driver is null and a.now_later='2' and a.region_code='$region_code' and c.status ='0' order by d.status_transaksi DESC")->result_array();
        // $newtrans = array_merge($newtrans,$newrl);
        
        
        if($kanigara == 1 && $berkah == 1 ){
        // echo "kanigara on";
        $new = $this->db->query("
        select a.*,b.*,c.*,d.status_transaksi,e.* 
        from transaksi a 
        ...
        //     left join status_transaksi d on c.status=d.id 
        //     left join fitur e on a.order_fitur=e.id_fitur 
        //     where a.id_driver is null and a.now_later='2' and a.region_code='100'and c.status ='0' order by d.status_transaksi DESC");
        //     return $newtrans;
        // }
    }
    ...
    public function getAlldriverby($job, $status,$statusjob, $area, $string, $ap="")
    {
        $this->db->select('config_driver.status as status_job');
        $this->db->select('driver_job.driver_job');
        $this->db->select('driver.*');
        ...
        $this->db->like('config_driver.status', $statusjob);        
    ...
    {   
        ...
        $this->db->select('saldo.*');
        $this->db->select('config_driver.status as status_job');
        $this->db->select('config_driver.latitude as lat');
        $this->db->select('config_driver.longitude as lng');
        $this->db->select('driver_job.driver_job');
    ...
    public function ubahdataid($data)
    {
        ...
        // if(count($this->get_driver_ap())>(int)$this->dashboard->get_driverseting()->ap){
         $this->db->set('ap', $data['ap']);
           $this->db->set('paskas', $data['paskas']);
        // }
