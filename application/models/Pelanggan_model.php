...
class Pelanggan_model extends CI_model
{
    ...
    public function terminalpricecardbyid()
    {
        //$area = 11;
        //$job = 8;
        $this->db->select('*');
        ...
        $this->db->where('id_area', "30");
        $this->db->where('id_fitur', "8");
    ...
    public function check_reff($reff)
    {
        $cek = $this->db->query("SELECT id FROM pelanggan where reff='$reff'");
        if ($cek->num_rows() > 0) {
            $permitted_chars = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ';
            $reff =  substr(str_shuffle($permitted_chars), 0, 5);
            return $reff;
        } else {
            return true;
        }
    ...
    public function getarea($point)
    {
        
        $this->db->select('id,AreaCoordinates,area_name');
        ...
        for ($i = 0; $i < $n; $i++)
        {
        $polygon = json_decode($area[$i]['AreaCoordinates']);
        
        $contains = $this->contains($point, $polygon)?'IN':'OUT';
        if($contains == 'IN'){
            return $area[$i]['id'];
            /*array(
                'id' => $area[$i]['id'],
    ...
    public function contains($point, $polygon)
    {
        ...
        for ($i = 0; $i < $n; $i++)
        {
            ...
            $koordi = (array) $polygon[$i];
            $koordj = (array) $polygon[$j];    
    ...
    public function fiturhome($area)
    {
        $this->db->select('*');
        $this->db->from('fitur');
        ...
        $this->db->limit('7');
        $fiturhome = $this->db->get()->result_array();
        $n = count($fiturhome);
        for ($i = 0; $i < $n; $i++)
        {
        $pricecard = $this->pricecardbyid($area, $fiturhome[$i]['id_fitur']);
        $fiturhome[$i]['biaya'] = $pricecard['distance'] ? $pricecard['distance'] : $fiturhome[$i]['biaya'] ;
        $fiturhome[$i]['biaya_minimum'] = $pricecard['base_fare'] ? $pricecard['base_fare'] : $fiturhome[$i]['biaya_minimum'] ;
        $fiturhome[$i]['wallet_minimum'] = $pricecard['minimum_wallet_amount'] ? $pricecard['minimum_wallet_amount'] : $fiturhome[$i]['wallet_minimum'] ;
        $fiturhome[$i]['surge'] = $pricecard['surge'] ? $pricecard['surge'] : 0 ;
        }
        return $fiturhome;
    }

    public function fiturhomeall($area)
    {
        $this->db->select('*');
        $this->db->from('fitur');
        $this->db->where('active', '1');
        $this->db->order_by('queue ASC');
        $fiturhomeall = $this->db->get()->result_array();
        $n = count($fiturhomeall);
        for ($i = 0; $i < $n; $i++)
        {
        $pricecard = $this->pricecardbyid($area, $fiturhomeall[$i]['id_fitur']);
        $fiturhomeall[$i]['biaya'] = $pricecard['distance'] ? $pricecard['distance'] : $fiturhomeall[$i]['biaya'] ;
        $fiturhomeall[$i]['biaya_minimum'] = $pricecard['base_fare'] ? $pricecard['base_fare'] : $fiturhomeall[$i]['biaya_minimum'] ;
        $fiturhomeall[$i]['wallet_minimum'] = $pricecard['minimum_wallet_amount'] ? $pricecard['minimum_wallet_amount'] : $fiturhomeall[$i]['wallet_minimum'] ;
        $fiturhome[$i]['surge'] = $pricecard['surge'] ? $pricecard['surge'] : 0 ;    
    }
        return $fiturhomeall;  
    }

    public function pricecardbyid($area, $id_fitur)
    {
        //$area = 11;
        //$job = 8;
        $this->db->select('*');
        ...
        $this->db->select('price_cards.base_fare, price_cards.surge, price_cards.distance');
        $this->db->from('price_cards');
        $this->db->where('status', '1');
        $this->db->where('id_area', $area);
        $this->db->where('id_fitur', $id_fitur);
    ...
    public function getwalletxx($id)
    {
        $this->db->select('wallet.*,'. 'callbackqris.reference_label,'.'callbackqris.data_qr as qrcode,'.'callbackqris.expired_time as expired');
        $this->db->from('wallet');
         $this->db->join('callbackqris', 'wallet.id = callbackqris.custom', 'left'); 
    ...
    public function getwallettrx($id)
    {
        $this->db->select('*');
    ...
    function get_biaya($area)
    {
        if($area == 1){
            ...
            foreach ($biaya as $row) {            
                $tempBiaya[] = array(
                    ...
                    'fitur' => $row['fitur'],
                    'biaya' => empty($row['distance']) ? $row['biaya'] : $row['distance'],
                    'surge' => 0,
                    'icon' => $row['icon'],
                    'home' => $row['home'],
                    'maks_distance' => $row['maks_distance'],
                    'icon_driver' => $row['icon_driver'],
                    'biaya_minimum' => empty($row['base_fare']) ?  $row['biaya_minimum'] : $row['base_fare'],
        ...
        $tempBiaya = array();
        $this->db->select('fitur.*,voucher.nilai, driver_job.icon as icon_driver,price_cards.distance as distance,price_cards.base_fare as base_fare,price_cards.surge as surge' );
        $this->db->from('fitur');
        $this->db->join('voucher', 'fitur.id_fitur = voucher.untuk_fitur', 'left');        
        $this->db->join('driver_job', 'fitur.driver_job = driver_job.id', 'left');
        $this->db->join('price_cards', 'fitur.id_fitur = price_cards.id_fitur', 'left'); 
        $this->db->where('price_cards.id_area', $area);
        $biaya = $this->db->get()->result_array();
        foreach ($biaya as $row) {            
            $tempBiaya[] = array(
                'id_fitur' => $row['id_fitur'],
                'fitur' => $row['fitur'],
                'biaya' => empty($row['distance']) ? $row['biaya'] : $row['distance'],
                'surge' => empty($row['surge']) ? 0 : $row['surge'],
                'icon' => $row['icon'],
                'home' => $row['home'],
                'maks_distance' => $row['maks_distance'],
                'icon_driver' => $row['icon_driver'],
    ...
    public function insert_transaksi($data_req, $data_jurnal, $status)
    {
        ...
        if ($this->db->affected_rows() == 1) {
            ...
            if ($this->db->affected_rows() == 1) {
                $data_jurnal['id_transaksi'] = $reqid;
                $this->db->insert('transaksi_jurnal', $data_jurnal);
    ...
    public function insert_transaksi_berkah($data_req,$data_jurnal,$datanotif,$datadriver)
    {
        ...
        if ($this->db->affected_rows() == 1) {
            ...
            $result=$this->notification->send_notif_req_berkah($datanotif,$datadriver);    
                return array(
                    'status' => true,
                    'data' => $get_data->result()
                );
    ...
    public function insert_transaksi_send($data_req, $dataDetail, $data_jurnal)
    {
    ...
    public function user_cancel_request($cond)
    {
        ...
        if ($id->row('status') == 1 || $id->row('status') == 2 || $id->row('status') == 0 || $id->row('status') == 12) {
            $data = array(
                'status' => '5'
            );
            if ($id2->row('home') == 4) {
    ...
    function get_driver_location_admin_car()
    {
        $datess = date("Y-m-d"); 
        $this->db->select(''
            . 'config_driver.id_driver,'
        ...
        $this->db->where('driver.status != 0');
        $this->db->where('driver.status != 3');
        //$this->db->like('driver.update_at', $datess);
        //$this->db->where('driver.update_at like 3');
        $this->db->where('driver.job = 8');
        $loc = $this->db->get();
        return $loc;
    }

    function get_driver_location_admin_bike()
    {
        $this->db->select(''
            . 'config_driver.id_driver,'
            . 'config_driver.latitude,'
    ...
    function all_transaksi($iduser)
    {
        ...
        $this->db->where('transaksi.aplikator',"SMARTRANS");
    ...
    public function getAlltransaksipickupbike()
    {
        $this->db->select('transaksi.*,' . 'driver.nama_driver,' . 'pelanggan.fullnama,' . 'history_transaksi.*,' . 'status_transaksi.*,' . 'fitur.icon,' . 'fitur.fitur');
        $this->db->from('transaksi');
        $this->db->join('history_transaksi', 'transaksi.id = history_transaksi.id_transaksi', 'left');
    ...
    public function getAlltransaksidestinationbike()
    {
        $this->db->select('transaksi.*,' . 'driver.nama_driver,' . 'pelanggan.fullnama,' . 'history_transaksi.*,' . 'status_transaksi.*,' . 'fitur.icon,' . 'fitur.fitur');
        $this->db->from('transaksi');
        $this->db->join('history_transaksi', 'transaksi.id = history_transaksi.id_transaksi', 'left');
